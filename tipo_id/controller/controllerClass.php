...
class controllerClass {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $rsp = modelClass::NewTipo_id($_POST['txtcod'], $_POST['txtDesc']);
            if ($rsp === true) {
    ...
    public function update() {
        if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
                ...
                if (count($certificate) > 0) {
                    ...
                    if (is_array($data)) {
                        if (count($data) > 0) {
                            ...
                            $args['desc_tipo_id'] = $data[0]['desc_tipo_id'];
                        }
                    } else {
                        $args['error'] = $data;
                        viewClass::renderHTML('error.php', $args);
            ...
            $data['desc_tipo_id'] = $_POST['txtDesc'];


            $rsp = modelClass::updateTipo_id($_GET['id'], $data);
            if ($rsp === true) {
    ...
    public function delete() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
            ...
            $rsp = modelClass::deleteTipo_id($_GET['id']);
            if ($rsp === true) {
